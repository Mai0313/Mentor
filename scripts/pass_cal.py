...
def process_log_file(file_path: Path) -> dict:
    ...
    with file_path.open(encoding="utf-8") as f:
        for line in f:
            ...
            if r"code_id:0	success." in line:
                counters["pass1"] += 1
                counters["pass2"] += 1
                counters["pass3"] += 1
                counters["pass4"] += 1
            ...
            elif r"code_id:1	success." in line:
                counters["pass2"] += 1
                counters["pass3"] += 1
                counters["pass4"] += 1
                counters["pass5"] += 1
                counters["total"] += 1
            elif r"code_id:2	success." in line:
                counters["pass3"] += 1
                counters["pass4"] += 1
                counters["pass5"] += 1
                counters["total"] += 1
            elif r"code_id:3	success." in line:
                counters["pass4"] += 1
                counters["pass5"] += 1
                counters["total"] += 1
            elif r"code_id:4	success." in line:
                counters["pass5"] += 1
                counters["total"] += 1
            else:
                # 依序檢查錯誤訊息
...
def display_model_scores(
    ...
    table.add_column("pass@2 Rate", justify="right")
    table.add_column("pass@3 Rate", justify="right")
    table.add_column("pass@4 Rate", justify="right")
    ...
    for score in sorted(scores, key=lambda x: x["problem_number"]):
        score["total"] if score["total"] > 0 else 1  # 避免除 0

        pass1_rate = score["pass1"] / num_per_task
        pass2_rate = score["pass2"] / num_per_task
        pass3_rate = score["pass3"] / num_per_task
        pass4_rate = score["pass4"] / num_per_task
