...
if abs(trigger_vin - trigger_vin2) <= 0.05:
    ...
    print(
        f"Trigger points: {trigger_vin:.5f}V and {trigger_vin2:.5f}V are not sufficiently different. Please use the positive feedback which the Rf should connect to the non-inverting input of the op-amp."
    )
    sys.exit(2)
elif vout[-1] - vout[0] < 2.5 or vout2[-1] - vout2[0] < 2.5:
    print(
        "The circuit does not function correctly. The output voltage does not vary more than Vdd/2."
    )
    sys.exit(2)
elif not np.all(np.diff(vout) >= 0) or not np.all(np.diff(vout2) >= 0):
    print(
        "The circuit does not function correctly. The output voltage variation does not monotonically increase with increasing input voltage."
    )
    sys.exit(2)
