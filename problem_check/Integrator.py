...
if vin_name != "":
    ...
    circuit.V(
        ...
        "Vin",
        circuit.gnd,
        f"dc {bias_voltage} PULSE({bias_voltage - 0.5} {bias_voltage + 0.5} 1u 1u 1u 10m 20m)",
    )
else:
    circuit.V(
        "in",
...
for element in circuit.elements:
    ...
    if (
        element.name.lower().startswith("cf")
        or element.name.lower().startswith("ccf")
        or element.name.lower().startswith("c1")
    ):
        c_name = element.name
...
if not np.isclose(slope, expected_slope, rtol=0.3):  # 30% tolerance
    print(
        f"The circuit does not function correctly as an integrator.\n"
