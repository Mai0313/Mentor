...
if vin_name != "":
    ...
    circuit.V(
        ...
        "Vin",
        circuit.gnd,
        f"dc {bias_voltage} PULSE({bias_voltage - 0.5} {bias_voltage + 0.5} 0 50m 50m 1n 100m)",
    )
else:
    circuit.V(
        "in",
...
for element in circuit.elements:
    if (
        element.name.lower().startswith("rf")
        or element.name.lower().startswith("rrf")
        or element.name.lower().startswith("r1")
    ):
        r_name = element.name
...
if (
    np.isclose(mean_peak - bias_voltage, -mean_trough + bias_voltage, rtol=0.2)
    and np.isclose(mean_peak - bias_voltage, 0.6, rtol=0.2)
    and is_square_wave(vout, mean_peak, mean_trough)
):  # 20% tolerance
    # print("The op-amp differentiator functions correctly.\n")
    # sys.exit(0)
    pass
elif not np.isclose(
    mean_peak - bias_voltage, -mean_trough + bias_voltage, rtol=0.2
) or not is_square_wave(vout, mean_peak, mean_trough):
    print(
        "The circuit does not function correctly as a differentiator.\n"
        "When the input is a triangle wave and the output is not a square wave.\n"
    )
    sys.exit(2)
else:
    print(
        f"The circuit does not function correctly as a differentiator.\n"
...
if np.isclose(mean_peak - bias_voltage, -mean_trough + bias_voltage, rtol=0.2) and np.isclose(
    mean_peak - bias_voltage, 0.6, rtol=0.2
):  # 20% tolerance
