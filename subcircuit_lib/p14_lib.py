...
class OpampResistanceLoad(SubCircuitFactory):
    ...
    def __init__(self):
        ...
        self.MOSFET(
            "1", "Drain1", "Vinp", "Source5", self.gnd, model="nmos_model", w=50e-6, l=1e-6
        )
        self.MOSFET(
            "2", "Drain2", "Vinn", "Source5", self.gnd, model="nmos_model", w=50e-6, l=1e-6
        )
        ...
        self.MOSFET(
            "5", "Source5", "Vbias2", self.gnd, self.gnd, model="nmos_model", w=100e-6, l=1e-6
        )
        # Second Stage: Common-Source with Active Load
        self.MOSFET(
            "6", "Vout", "Drain1", self.gnd, self.gnd, model="nmos_model", w=100e-6, l=1e-6
        )
