...
class SingleStageDiffCommonSourceOpamp(SubCircuitFactory):
    ...
    def __init__(self):
        ...
        self.MOSFET(
            "1", "Vout", "Vinp", "Source3", "Source3", model="nmos_model", w=50e-6, l=1e-6
        )  # Output taken from Drain1
        self.MOSFET(
            "2", "Drain2", "Vinn", "Source3", "Source3", model="nmos_model", w=50e-6, l=1e-6
        )
        # Tail Current Source with adjusted parameters for proper activation
        self.MOSFET(
            "3", "Source3", "Vbias", self.gnd, self.gnd, model="nmos_model", w=100e-6, l=1e-6
        )
        # Load Resistors
        self.R(
            "1", "Vout", "Vdd", 1 @ u_kΩ
        )  # Connected to Vout for correct output node identification
